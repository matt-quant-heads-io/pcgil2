...
class FullyConvPolicyBigMap(ActorCriticPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, **kwargs):
        ...
        with tf.compat.v1.variable_scope("model", reuse=kwargs['reuse']):
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            # print(f"self.[self.deterministic_action, self.value_flat, self.neglogp] is {[self.deterministic_action, self.value_flat, self.neglogp]}")
            # print(f"self.obs_ph: obs is {self.obs_ph}:{obs}")
            action, value, neglogp = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp],
                                                   {self.obs_ph: obs})
        else:
            # print(f"[self.action, self.value_flat, self.neglogp]is {[self.action, self.value_flat, self.neglogp]}")
            # print(f"self.obs_ph: obs is {self.obs_ph}:{obs}")
    ...
    def value(self, obs, state=None, mask=None):
        # print(f"self.value_flat is {self.value_flat}")
        # print(f"self.obs_ph is {self.obs_ph}")
        # print(f"obs is {obs}")
