...
def read_action_seq(path):
    print(f"inside of read_action_seq")
    action_sequence = []
    final_action_sequence = []
    with open(path, 'r') as f:
        action_sequence = f.readlines()
    for line in action_sequence:
        new_line = []
        line_wo_ret = line.split('\n')[0].split(' ')
        for el in line_wo_ret:
            num = int(el)
            new_line.append(num)
        final_action_sequence.append(new_line)
...
"""
class ZeldaProblemOrderless(Problem):
    """
    ...
    def __init__(self):
        ...
        self._action_sequence_path = '../expert_trajectories_wide_orderless/init_maps_level{}/action_space_{}.txt'.format(self._good_level_seed, self._start_map_seed)
        self._init_map_path = '../expert_trajectories_wide_orderless/init_maps_level{}/starting_map_{}.txt'.format(self._good_level_seed, self._start_map_seed)


        # TODO: implement self.read_action_seq
        self._action_sequence_int = read_action_seq(self._action_sequence_path)
        # TODO: store self._action_sequence_int
        self._init_map = self.to_2d_array_level(self._init_map_path)

        self._hamming_dist = None
    ...
    def get_stats(self, action):
        # update self.action_sequence_str, self.action_sequence_int
        if action in self._action_sequence_int:
            self._action_sequence_int.remove(action)

        return len(self._action_sequence_int), self._action_sequence_int
    ...
    def get_reward(self, new_stats, old_stats):
        if new_stats < old_stats:
            rew_val = int(1.0/(len(self._action_sequence_int) + 1.0) *10.0)
            return rew_val
        else:
            return -10
    ...
    def get_episode_over(self):
        return len(self._action_sequence_int) == 0
    ...
    def get_debug_info(self):
        return {
            "action_seq_len": len(self._action_sequence_int)
        }
