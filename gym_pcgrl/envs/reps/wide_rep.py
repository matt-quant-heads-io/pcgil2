...
class WideRepresentation(Representation):
    ...
    def update(self, action):
        # print(f"Inside of action")
        # print(f"action is {action}")
        # print(f"self._map[action[1]][action[0]] != action[2] is {self._map[action[1]][action[0]] != action[2]}")
        # print(f"action is {action}")
        # print(f"self.self._map[action[1]][action[0]] is {self._map[action[1]][action[0]]}")
        new_oh = [0]*8
        new_oh[action[2]] = 1
        change = [0,1][self._map[action[1]][action[0]] != new_oh]
        # print(f"after change")
        # print(f"change is {change}")
        # print(f"change is {change}")

        self._map[action[1]][action[0]] = new_oh
